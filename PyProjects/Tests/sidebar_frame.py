...
def fill():
    if expanded:  # If the frame is exanded
        ...
        home_b.config(text='Home', image=home, font=(0, 21))
        set_b.config(text='Settings', image=settings, font=(0, 21))
        ring_b.config(text='Bell Icon', image=ring, font=(0, 21))
    else:
...
home = ImageTk.PhotoImage(Image.open('ana.png').resize((40, 40), Image.ANTIALIAS))
settings = ImageTk.PhotoImage(Image.open('ashe.png').resize((40, 40), Image.ANTIALIAS))
ring = ImageTk.PhotoImage(Image.open('baptiste.png').resize((40, 40), Image.ANTIALIAS))
